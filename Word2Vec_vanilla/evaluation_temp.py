...
def compositionality(emb, vocabulary, word_to_index, index_to_word, eval_point = 1000, k = 10):
    embedding_norm = np.linalg.norm(emb, axis=1)
    emb = emb / embedding_norm[:, None]
    q = [['german','airlines'],['thailand','holiday'],['korean','president'],['spanish','dance'],['korean','musician'],['china','liquor']]
    near_containter = []
    for pair in q:
...
if __name__ == '__main__':
    ...
    # make_evaluation_tokens(word_to_index)

    # topk = find_nearest_k(emb, ["tokyo","japan","korea"], word_to_index, normalized = False, k = 10)
    # for i in topk:
        # print(index_to_word[i])

    # eval_monitor(emb, vocabulary, word_to_index, eval_point = 1000, k = 1) #eval_point에는 eval_monitor이루어진 step 넣기 (저장용)

    compositionality(emb, vocabulary, word_to_index, index_to_word, eval_point = 1000, k = 5)

    # for f in os.listdir('./results/monitor'):
    #     temp = pickle.load(open(f'./results/monitor/{f}', 'rb'))
    #     print(f)
    #     print(temp)

    # similar_word_temp(emb, normalized = False)
    
    # result = eval_overall(emb, vocabulary, word_to_index, index_to_word, eval_point = 123, k = 1)
    # print(f'avg % of correct words : {result}')


    # similar_word(emb)
    # king = emb[word_to_index['king']]
    # man = emb[word_to_index['man']]
    # woman = emb[word_to_index['woman']]
    # result = king - man + woman
    # queen = emb[word_to_index['queen']]
    #
    # print(np.linalg.norm(man - woman))
    # print(np.linalg.norm((result - queen)))
    # print(np.linalg.norm(king - queen))
    # print(np.linalg.norm(man - queen))